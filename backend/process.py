...
def read_community_fisherman_data():
    ...
    for fisherman in fishermen:
        ...
        for fish in selected_fish:
            ...
            allocated_fish.append((fish, allocated_quantity, actual_haul, difference))

        # Write allocated fish data to community_fisherman_data
        fisherman_column_name = next_week_date.strftime('%m/%d/%Y')
        for fish_data in allocated_fish:
            community_fisherman_data.at[fishermen.index(fisherman), fisherman_column_name] = f"{fish_data[1]};{fish_data[2]};{fish_data[3]}"
